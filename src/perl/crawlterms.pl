...
if(!$query->execute()){
$query = $dbh->prepare('create table terminology (term varchar(100), definition varchar(500), parentterm varchar(100), primary key(term, parentterm)) CHARACTER SET utf8 engine=innodb');
$query->execute();
}
