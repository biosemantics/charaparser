...
public class NumericalChunkProcessor extends AbstractChunkProcessor {
	...
	protected List<Element> processChunk(Chunk chunk, ProcessingContext processingContext) {
		...
		String character = text.indexOf("some_measurement") >= 0 ? "some_measurement" : null;
		if(character ==null){
		...
		if(character ==null){
			...
			else character = content.indexOf('/') > 0 || content.indexOf('%') > 0 ? "some_measurement_or_shape" : "some_measurement";
		}
	...
	String content = rest.replace(modifier2, "").replaceAll("(\\{|\\})", "").trim();
	modifier2 = modifier2.replaceAll("(\\w+\\[|\\]|\\{|\\})", "").trim();
	ArrayList<Element> chars = annotateNumericals(content, text.indexOf("some_measurement")>=0 || content.indexOf('/')>0 || content.indexOf('%')>0 || content.indexOf('.')>0? "some_measurement" : null, (modifier1+";"+modifier2).replaceAll("(^\\W|\\W$)", ""), lastStructures(), resetfrom);
