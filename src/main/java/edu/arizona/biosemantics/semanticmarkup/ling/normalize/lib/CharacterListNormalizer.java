...
public class CharacterListNormalizer{
	...
	ArrayList<String> lookupCharacters(String str, boolean markadv) {
		...
		for(int i = chunkedTokens.size()-1; i>=0; i--){
			...
			}else if(characterKnowledgeBase.isCategoricalState(word) && !characterKnowledgeBase.isEntity(word)) {
	 ...
	 * for example: keeled , elliptic to broadly ovate to shape~list~keeled~punct~elliptic~to~broadly~ovate
	 * example 2: not keeled , elliptic to broadly ovate to shape~list~not~keeled~punct~elliptic~to~broadly~ovate //"rarely" will be treated the same way as "not"
	 * example 3: not keeled or ovate to shape~list~not~keeled~or~ovate //because of "or", the modifer needs to be applied to all states in the or-list
	 *
	 ...
	 * @param characterTokensReversed "# color position"
