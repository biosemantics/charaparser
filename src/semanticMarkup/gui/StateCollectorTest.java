...
public class StateCollectorTest extends StateCollector {
	...
	public StateCollectorTest(Connection conn, String tableprefix, boolean filtered, String glosstable, Display display, StyledText charLog,
		...
		super(conn, tableprefix, glosstable, display, charLog, organStateKnowledgeBase, inflector);
	...
	public StateCollectorTest(Connection conn, String tableprefix, ArrayList<String> knownstates, boolean filtered, String glosstable, Display display, StyledText charLog, 
			IOrganStateKnowledgeBase organStateKnowledgeBase, IPOSKnowledgeBase posKnowledgeBase, IInflector inflector) {
		super(conn, tableprefix, knownstates, glosstable, display, charLog, organStateKnowledgeBase, inflector);
		this.filtered = filtered;
		this.posKnowledgeBase = posKnowledgeBase;
	}
	...
	protected boolean isAdv(String word){
		...
		if(word.compareTo(wordc) != 0) {
			if(posKnowledgeBase.isAdjective(word) || posKnowledgeBase.isAdverb(word + "e")){
		...
		if(posKnowledgeBase.getMostLikleyPOS(wordc) !=null && posKnowledgeBase.getMostLikleyPOS(wordc).equals(POS.RB)){
